...
# vim: et sts=2 sw=2
#
# A collection of functions to repair and modify a Steam Device installation.
...
prompt_reboot()
{
  prompt_step "Action Complete" "${1}\n\nChoose Proceed to reboot the Steam Device now, or Cancel to stay in the repair image."
  [[ $? = 0 ]] || exit 1
  if [[ ${POWEROFF:-} ]]; then
...
{
  readvar HELPMSG << EOD
This tool can be used to reinstall or repair your SteamOS installation on a Steam Device.

Possible targets:
    all : permanently destroy all data on your Steam Device, and reinstall SteamOS.
    system : reinstall SteamOS on the Steam Device system partitions.
    home : remove games and personalization from the Steam Device.
...
case "${1-help}" in
all)
  prompt_step "Reimage Steam Device" "This action will reimage the Steam Device.\nThis will permanently destroy all data on your Steam Device and reinstall SteamOS.\n\nThis cannot be undone.\n\nChoose Proceed only if you wish to clear and reimage this device."
  ...
  ;;
system)
  prompt_step "Reinstall SteamOS" "This action will reinstall SteamOS on the Steam Device, while attempting to preserve your games and personal content.\nSystem customizations may be lost.\n\nChoose Proceed to reinstall SteamOS on your device."
  writeOS=1
  repair_steps
  prompt_reboot "SteamOS reinstall complete."
  ;;
home)
  prompt_step "Delete local user data" "This action will reformat the home partitions on your Steam Device.\nThis will destroy downloaded games and all personal content stored on the Device, including system configuration.\n\nThis action cannot be undone.\n\nChoose Proceed to reformat all user home partitions."
  writeHome=1
  repair_steps
